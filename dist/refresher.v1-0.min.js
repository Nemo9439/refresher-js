!function(){const n="refresher-js-toast",t="#004dff",e="New version is available",o="Please refresh the page",r="Refresh",s=120;let i=!1;createToastElement=()=>{const t=document.createElement("div");t.id=n;var e=`
  ...
  `},openToastLater=()=>{setTimeout(()=>{openToast()},6e4*c)},refresh=()=>{location.reload(!0)},closeToast=()=>{const t=getToastElement();t.classList.remove("opened"),setTimeout(()=>{t.remove()},1e3),openToastLater()},openToast=()=>{if(dispatchOpenToastEvent(),!getToastElement()&&!m){const t=createToastElement();setTimeout(()=>{t.classList.add("opened")},1e3)}},getToastElement=()=>document.getElementById(n),dispatchOpenToastEvent=()=>{document.dispatchEvent(new Event("refresher-js-open-toast"))},getMainScript=t=>{const e=[...t.getElementsByTagName("script")];t=e.filter(t=>t?.src?.includes("main."))?.[0];return(t||e.filter(t=>t?.src?.includes("app."))?.[0])?.src},getRefresherScriptTag=()=>document.getElementById("refresher-js-script"),handleCustomFunction=async()=>{await window[g]()&&(openToast(),clearInterval(x))},subscribeToActivityEvents=()=>{document.onvisibilitychange=()=>{"visible"===document.visibilityState&&(i=!0)},onclick=()=>{i=!0}};const a=new XMLHttpRequest,l=(a.onload=t=>{t=(new DOMParser).parseFromString(t.target.response,"text/html"),t=getMainScript(t);h!==t&&(openToast(),clearInterval(x),h=t)},getRefresherScriptTag()),c=Number(l.getAttribute("data-polling-interval-in-minutes"))??s,p=l.getAttribute("data-title-text")??e,d=l.getAttribute("data-subtitle-text")??o,u=l.getAttribute("data-primary-color")??t,f=l.getAttribute("data-refresh-button-text")??r,m="true"===l.getAttribute("data-disable-toast"),g=l.getAttribute("data-custom-function-name"),b=window.location.href;let h=getMainScript(window.document);subscribeToActivityEvents();const x=setInterval(async()=>{if(!getToastElement()&&i){if(i=!1,g)return handleCustomFunction();a.open("GET",b),a.send()}},6e4*c)}();
